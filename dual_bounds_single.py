...
def _maybe_print_ot_progress(
    ...
    if diagnostics is not None:
        diag_msg = (
            ...
            f" ot_runtime={diagnostics.runtime_seconds:.3f}s"
        )
    print(f"[ot] {label}: {done}/{total}{diag_msg}", flush=True)
...
def _first_term_summands_for_eval_units(
    ...
    for row, i in enumerate(eval_idx):
        ...
        lower_c0 = float(probs0_eval[row] @ ot_pair.lower_u0)
        lower_c1 = float(probs1_eval[row] @ ot_pair.lower_u1)
        upper_c0 = float(probs0_eval[row] @ ot_pair.upper_u0)
        upper_c1 = float(probs1_eval[row] @ ot_pair.upper_u1)

        if w == 0:
            lower_obs = ot_pair.lower_u0[atom]
            upper_obs = ot_pair.upper_u0[atom]
            ...
            lower_obs = ot_pair.lower_u1[atom]
            upper_obs = ot_pair.upper_u1[atom]
...
def print_row_summary(row: dict) -> None:
    msg = (
        ...
        f"ot={row['ot_runtime_seconds']:.3f}s "
