...
def _make_adaboost_classifier():
	"""
	Construct an AdaBoost classifier across sklearn versions.

	sklearn < 1.6 accepts algorithm='SAMME', while newer versions removed the
	argument and use SAMME as the only supported algorithm.
	"""
	kwargs = {}
	if 'algorithm' in inspect.signature(sklearn.ensemble.AdaBoostClassifier).parameters:
		kwargs['algorithm'] = 'SAMME'
	return sklearn.ensemble.AdaBoostClassifier(**kwargs)
...
class TestGenericDualBounds(context.DBTest):
	...
	def test_var_ite_oracle(self):
		...
		for dist0, dist1, discrete, support in zip(
			...
			y1 = y1_dists.ppf(U); y0l = y0_dists.ppf(U); y0u = y0_dists.ppf(1-U)  ##the test can't be extended to vectors easily
	...
	def test_generic_vector_discrete_oracle(self):
		np.random.seed(123)
		n = 12
		support = np.array([
			[0, 0],
		...
		yvals = np.repeat(support[None, :, :], n, axis=0)
		probs0 = np.random.uniform(size=(n, len(support)))
		probs0[:, 0] = 0
		probs0 /= probs0.sum(axis=1, keepdims=True)
		probs1 = np.random.uniform(size=(n, len(support)))
		probs1[:, -1] = 0
		probs1 /= probs1.sum(axis=1, keepdims=True)
		y0_dists = BatchedCategorical(vals=yvals, probs=probs0)
		y1_dists = BatchedCategorical(vals=yvals, probs=probs1)
		pis = np.ones(n) / 2
		W = np.random.binomial(1, pis, size=n)
		Y = y0_dists.rvs()
		Y[W == 1] = y1_dists.rvs()[W == 1]

		gdb = db.generic.DualBounds(
			f=lambda y0, y1, x: np.sum(y1 - y0, axis=-1) >= 1,
			outcome=Y,
			treatment=W,
		...
		self.assertEqual(gdb.y0_vals.shape, (n, len(support), 2))
		self.assertEqual(gdb.y0_probs.shape, (n, len(support)))
		self.assertTrue(np.all(np.isfinite(gdb.objvals)))
		...
		self.assertTrue(np.all(np.isfinite(gdb.estimates)))

	def test_generic_vector_discrete_crossfit(self):
		np.random.seed(123)
		n = 160
		support = np.array([
			[0, 0],
		...
		gdb = db.generic.DualBounds(
			f=lambda y0, y1, x: np.all(y0 == y1, axis=-1).astype(float),
			outcome=Y,
			treatment=W,
			covariates=X,
			propensities=pis,
		...
		gdb.fit(nfolds=3, verbose=False)
		self.assertTrue(np.all(np.isfinite(gdb.estimates)))
		np.testing.assert_allclose(
			gdb.estimates,
			gdb.objvals.mean(axis=1),
			atol=0.5,
			err_msg="vector discrete AIPW estimates are far from plug-in objective values",
		)
		self.assertIn('Accuracy', gdb.eval_outcome_model().index)
	...
	def test_fit_propensity_scores(self):
		...
		propensity_models.append(_make_adaboost_classifier())
