...
def get_default_model(discrete, support, outcome_model=None, **model_kwargs):
	...
	support_arr = None if support is None else np.asarray(support)
	if support_arr is not None and support_arr.ndim == 2 and support_arr.shape[1] == 1:
		support_arr = support_arr[:, 0]
	if support_arr is not None and support_arr.ndim == 1 and set(support_arr.tolist()) == set([0, 1]):
		return dist_reg.BinaryDistReg(
			model_type=outcome_model, **model_kwargs
		)
	else:
		return dist_reg.MulticlassDistReg(
			model_type=outcome_model, support=support_arr, **model_kwargs
		)
...
def _normalize_support_for_outcome(support, outcome_dim):
	if support is None:
		return None
	support = np.asarray(support)
	if outcome_dim == 1:
		if support.ndim == 2 and support.shape[1] == 1:
			return support[:, 0]
		if support.ndim != 1:
			raise ValueError("scalar outcomes require support to have shape (m,)")
	else:
		if support.ndim != 2 or support.shape[1] != outcome_dim:
			raise ValueError(
				f"vector outcomes require support to have shape (m, {outcome_dim})"
			)
	return support

def _find_support_row(vals, y):
	vals = np.asarray(vals)
	y = np.asarray(y)
	if vals.ndim == 1:
		matches = vals == y
	else:
		matches = np.all(vals == y.reshape(1, -1), axis=1)
	if not np.any(matches):
		raise ValueError("realized outcome is not present in the discrete support")
...
class DualBounds:
	...
	outcome : np.array | pd.Series
		n-length array of scalar outcome measurements (Y), or an
		(n, d)-shaped array of finite discrete vector outcomes.
	...
	support : np.array
		...
		Defaults to ``None`` (inferred from the data). Scalar support has
		shape (m,); vector support has shape (m, d).
	outcome_dim : int
		Optional dimension of vector-valued outcomes. If provided, it is
		checked against ``outcome`` and ``support``.
	...
	def __init__(
		...
		**model_kwargs,
		...
		if self.y.ndim == 0:
			raise ValueError("outcome should be an array")
		if len(self.y.shape) > 1:
			if len(self.y.shape) == 2 and self.y.shape[1] == 1:
				self.y = self.y.flatten()
				inferred_outcome_dim = 1
			elif len(self.y.shape) == 2:
				inferred_outcome_dim = self.y.shape[1]
			else:
				raise ValueError("outcome should have shape (n,) or (n, d)")
		else:
			inferred_outcome_dim = 1
		if outcome_dim is not None and outcome_dim != inferred_outcome_dim:
			raise ValueError(
				f"outcome_dim={outcome_dim} does not match outcome dimension {inferred_outcome_dim}"
			)
		self.outcome_dim = inferred_outcome_dim
		self.n = len(self.y)
		...
		self.support = _normalize_support_for_outcome(
			support=self.support, outcome_dim=self.outcome_dim,
		)
		if self.outcome_dim > 1 and not self.discrete:
			raise ValueError("vector outcomes are only supported with discrete=True")
	...
	def _apply_ot_fn(
		...
		"""
		if getattr(self, "outcome_dim", 1) > 1:
			y0 = np.asarray(y0)
			y1 = np.asarray(y1)
			if y0.ndim == 1:
				if y0.shape[0] != self.outcome_dim:
					raise ValueError("y0 has incorrect vector outcome dimension")
				y0 = y0.reshape(1, self.outcome_dim)
			if y1.ndim == 1:
				if y1.shape[0] != self.outcome_dim:
					raise ValueError("y1 has incorrect vector outcome dimension")
				y1 = y1.reshape(1, self.outcome_dim)
			orig = fn(
				y0=y0.reshape(y0.shape[0], 1, self.outcome_dim),
				y1=y1.reshape(1, y1.shape[0], self.outcome_dim),
				**kwargs
			)
			return orig + np.zeros((y0.shape[0], y1.shape[0]))
	...
	def _ensure_primal_feasibility(
		...
		if np.asarray(y0_vals).ndim == 1 and np.asarray(y1_vals).ndim == 1:
			...
			obj = cp.sum_squares(y0p - y0_probs) + cp.sum_squares(y1p - y1_probs)
	...
	def _solve_single_instance(
		...
		**kwargs,
		...
		if dual_strategy == 'ot':
			...
			objval, log = ot.lp.emd2(##the solver is in the environment POT)
				a=probs0,
				b=probs1,
				M=fvals,##the cost function
				log=True,
				**kwargs,
			)
			nu0, nu1 = log['u'], log['v'] 
		...
		if dual_strategy in ['lp', 'qp', 'se']:
			...
			##$\mathbb{E}\left[\nu_0(Y(0)) \mid X_i\right] + \mathbb{E}\left[\nu_1(Y(1)) \mid X_i\right]$
	...
	def _interpolate_and_ensure_feas(
		...
		if not self.discrete:
			...
			if self.outcome_dim > 1:
				j0 = _find_support_row(self.y0_vals[i], yi)
				j1 = _find_support_row(self.y1_vals[i], yi)
			else:
	...
	def plot_dual_variables(self, i=0):
		...
		"""
		if self.outcome_dim > 1:
			raise ValueError("plot_dual_variables is only supported for scalar outcomes")
	...
	def diagnostics(self, plot=False, aipw=True):
		...
		if plot:
			if self.outcome_dim > 1:
				raise ValueError("diagnostic plots are only supported for scalar outcomes")
			import matplotlib.pyplot as plt
			fig, axes = plt.subplots(1, 2, figsize=(10, 4))
	...
	def eval_outcome_model(self):
		...
		support_arr = None if self.support is None else np.asarray(self.support)
		is_binary_support = (
			support_arr is not None and
			support_arr.ndim == 1 and
			set(support_arr.tolist()) == set([0, 1])
		)
		if self.discrete and not is_binary_support and self.oos_dist_preds is not None:
			return dist_reg._evaluate_model_predictions(
				y=self.y, haty=self.oos_dist_preds
			)
		return dist_reg._evaluate_model_predictions(
...
def _plug_in_no_covariates(
	...
	if y.ndim == 2:
		y0_vals, y0_inverse = np.unique(y0_vals, axis=0, return_inverse=True)
		y0_probs = np.bincount(y0_inverse, weights=y0_probs, minlength=len(y0_vals))
		y0_probs = y0_probs / y0_probs.sum()
		y1_vals, y1_inverse = np.unique(y1_vals, axis=0, return_inverse=True)
		y1_probs = np.bincount(y1_inverse, weights=y1_probs, minlength=len(y1_vals))
		y1_probs = y1_probs / y1_probs.sum()
		if len(y0_vals) > max_nvals or len(y1_vals) > max_nvals:
			raise ValueError(
				"vector outcome support is too large for plug_in_no_covariates; "
				"provide a smaller support or increase max_nvals"
			)
		fvals = f(y0_vals[:, None, :], y1_vals[None, :, :], x=0)
...
def plug_in_no_covariates(
	...
	if outcome.ndim == 1:
		...
		data = np.concatenate(
			[outcome, treatment.reshape(-1, 1), propensities.reshape(-1, 1)],
			axis=1,
		)
		func = lambda data: _plug_in_no_covariates(
			f=f,
