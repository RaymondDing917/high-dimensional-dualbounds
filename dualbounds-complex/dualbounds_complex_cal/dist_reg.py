...
def _concat_batched_categoricals(dists):
	if isinstance(dists, BatchedCategorical):
		return dists.vals, dists.probs
	if isinstance(dists, list) and len(dists) > 0 and isinstance(dists[0], BatchedCategorical):
		return (
			np.concatenate([x.vals for x in dists], axis=0),
			np.concatenate([x.probs for x in dists], axis=0),
		)
	return None, None

def _evaluate_categorical_predictions(y, haty, tol=1e-9):
	vals, probs = _concat_batched_categoricals(haty)
	if vals is None:
		return None
	...
	matches = np.all(val_rows == y_rows[:, None, :], axis=2)
	actual_probs = np.sum(probs * matches, axis=1)
	pred_inds = np.argmax(probs, axis=1)
	pred_rows = val_rows[np.arange(n), pred_inds]
	accuracy = np.mean(np.all(pred_rows == y_rows, axis=1))
	likelihood_geom = np.exp(np.log(np.clip(actual_probs, tol, 1)).mean())

	if y.ndim == 1:
		_, inverse = np.unique(y, return_inverse=True)
	else:
		_, inverse = np.unique(y, axis=0, return_inverse=True)
	counts = np.bincount(inverse)
	if n <= 1:
		null_probs = np.ones(n)
		null_accuracy = 1.0
	else:
		null_probs = (counts[inverse] - 1) / (n - 1)
		null_accs = np.zeros(n)
		for i in range(n):
			loo_counts = counts.copy()
			loo_counts[inverse[i]] -= 1
			max_count = loo_counts.max()
			ties = np.sum(loo_counts == max_count)
			if loo_counts[inverse[i]] == max_count:
				null_accs[i] = 1 / ties
		null_accuracy = null_accs.mean()
	null_likelihood_geom = np.exp(np.log(np.clip(null_probs, tol, 1)).mean())
	return pd.DataFrame(
		np.array([
			[accuracy, null_accuracy],
			[likelihood_geom, null_likelihood_geom],
		]),
		index=['Accuracy', 'Likelihood (geom. mean)'],
		columns=['Model', 'No covariates'],
	)

def _evaluate_model_predictions(y, haty, tol=1e-9):
	cat_results = _evaluate_categorical_predictions(y=y, haty=haty, tol=tol)
	if cat_results is not None:
		return cat_results

	y = np.asarray(y)
	haty = np.asarray(haty)
	if y.ndim == 2:
		n = len(y)
		resids = y - haty
		ybar_loo = (n * y.mean(axis=0) - y) / (n-1)
		resids_null = y - ybar_loo
		rmse = np.sqrt(np.mean(np.sum(resids**2, axis=1)))
		rmse_null = np.sqrt(np.mean(np.sum(resids_null**2, axis=1)))
		mae = np.mean(np.sum(np.abs(resids), axis=1))
		mae_null = np.mean(np.sum(np.abs(resids_null), axis=1))
		return pd.DataFrame(
			np.array([
				[rmse, rmse_null],
				[mae, mae_null],
			]),
			index=['Vector RMSE', 'Vector MAE'],
			columns=['Model', 'No covariates'],
		)
	...
	def select_model(
		...
		**kwargs
		...
		for i, model in enumerate(models):
			...
			if (
				len(oos_preds) > 0
				and isinstance(oos_preds[0], BatchedCategorical)
				and (isinstance(model, MulticlassDistReg) or np.asarray(y).ndim == 2)
			):
				haty = oos_preds
			else:
				haty = np.concatenate([x.mean() for x in oos_preds])
			results = _evaluate_model_predictions(
				y, haty=haty
			)
			if 'Out-of-sample R^2' in results.index:
				r2s[i] = results.loc['Out-of-sample R^2', 'Model'].item()
			else:
				r2s[i] = results.loc['Accuracy', 'Model'].item()
...
class MulticlassDistReg(DistReg):
	"""
	Multiclass regression for finite discrete outcomes.

	This treats each scalar support value, or each row of a vector-valued
	support array, as one joint outcome category.
	"""
	def __init__(
		self,
		model_type: Union[str, sklearn.base.BaseEstimator] = 'logistic',
		...
		**model_kwargs,
		...
		self.mtype_raw = model_type
		self.model_type = parse_model_type(model_type, discrete=True)
		self.support = None if support is None else self._normalize_support(support)
		self.how_transform = str(how_transform).lower()
		self.model_kwargs = model_kwargs
	...
	def _normalize_support(self, support):
		...
		if support.ndim == 2 and support.shape[1] == 1:
			return support[:, 0]
		if support.ndim not in [1, 2]:
			raise ValueError("support must have shape (m,) or (m, d)")
		return support

	def _infer_support(self, y):
		y = np.asarray(y)
		if y.ndim == 1:
			return np.unique(y)
		if y.ndim == 2:
	...
	def _labels_from_y(self, y):
		...
		if self.support.ndim == 1:
			if y.ndim == 2 and y.shape[1] == 1:
				y = y[:, 0]
			if y.ndim != 1:
				raise ValueError("scalar support requires y to have shape (n,)")
			matches = self.support.reshape(1, -1) == y.reshape(-1, 1)
		else:
			if y.ndim != 2 or y.shape[1] != self.support.shape[1]:
				raise ValueError("vector support requires y to have shape (n, d)")
			matches = np.all(self.support[None, :, :] == y[:, None, :], axis=2)
		if not np.all(matches.any(axis=1)):
			raise ValueError("y contains values not present in support")
		return np.argmax(matches, axis=1)

	def _batched_support(self, n):
		if self.support.ndim == 1:
			return np.tile(self.support, (n, 1))
		return np.repeat(self.support[None, :, :], n, axis=0)
	...
	def fit(
		...
		if self.support is None:
			self.support = self._normalize_support(self._infer_support(y))
		self.nclasses = len(self.support)
		labels = self._labels_from_y(y)
		features = self.feature_transform(W=W, X=X, Z=Z)

		unique_labels = np.unique(labels)
		if len(unique_labels) == 1:
			self.constant_class = unique_labels[0]
			self.model = None
			return
		self.constant_class = None
		self.model = self.model_type(**self.model_kwargs)
		fit_kwargs = dict() if sample_weight is None else dict(sample_weight=sample_weight)
		self.model.fit(features, labels, **fit_kwargs)
	...
	def predict_proba(
		...
		if self.constant_class is not None:
			probs[:, self.constant_class] = 1
			return probs

		raw_probs = self.model.predict_proba(
			self.feature_transform(W, X, Z=Z)
		)
		for j, cls in enumerate(self.model.classes_.astype(int)):
			probs[:, cls] = raw_probs[:, j]
		return probs

	def predict(self, X: np.array, W: np.array, Z: Optional[np.array]=None):
		return BatchedCategorical(
			vals=self._batched_support(len(X)),
			probs=self.predict_proba(X=X, W=W, Z=Z),
		)
