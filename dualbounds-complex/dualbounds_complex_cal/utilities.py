...
def _sort_disc_dist(vals, probs):
	...
	Parameters
	----------
	vals : (n, nvals)-shaped or (n, nvals, d)-shaped array.
	probs : (n, nvals)-shaped array. probs.sum(axis=1) == 1

	Returns
	-------
	vals_new : np.array
		For scalar support, ``vals`` sorted so that vals[i] is in increasing
		order. For vector support, the original support order is preserved.
	...
	if vals.ndim == 3:
		return vals, probs
	if vals.ndim != 2:
		raise ValueError("vals must have shape (n, nvals) or (n, nvals, d)")
...
class BatchedCategorical:
	...
	----------
	vals : np.array
		(n, nvals)-shaped array of scalar supports or (n, nvals, d)-shaped
		array of vector supports.
	probs : np.array
	...
	def __init__(
		...
		if vals.ndim not in [2, 3]:
			raise ValueError("vals must have shape (n, nvals) or (n, nvals, d)")
		if probs.ndim != 2:
			raise ValueError("probs must have shape (n, nvals)")
		if vals.shape[:2] != probs.shape:
			raise ValueError("vals.shape[:2] must equal probs.shape")
		# sort
		self.n, self.nvals = probs.shape
		self.val_shape = vals.shape[2:]
	...
	def mean(self):
		...
		-------
		mu : np.array
			n-length mean of scalar discrete distributions or (n, d)-shaped
			mean of vector-valued discrete distributions.
		"""
		if self.vals.ndim == 2:
			return np.sum(self.vals * self.probs, axis=1)
		return np.sum(self.vals * self.probs[:, :, None], axis=1)
	...
	def ppf(self, q):
		...
		qvals = np.zeros((m, self.n) + self.val_shape)
	...
	def rvs(self):
		...
		-------
		y : array
			n-length array for scalar categorical distributions or (n, d)-shaped
			array for vector-valued categorical distributions.
		"""
...
def parse_dist(  
	...
	if not isinstance(dist, str):
		return dist
    ##`loc:means mean ;scale means variance`
	# Parse
	if dist == 'constant':  
	...
	if dist == 'cauchy':  ##very heavy tails 
		return stats.cauchy(loc=loc, scale=scale)
	if dist == 'laplace': ## spike and with heavy tails
		return stats.laplace(loc=loc, scale=scale)
	if dist in ['uniform', 'unif']: ## the noise has bounds(mild condition)
		return stats.uniform(loc=loc-scale/2, scale=scale)
	if dist in ['expo', 'expon', 'exponential']:  ##heavy tail in one direction
